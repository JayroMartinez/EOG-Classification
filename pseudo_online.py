...
global play_labels, play_times, predicted_labels, predicted_times, action_time
...
def play_sound(pl_lab):
	song_name = "./audio/"+pl_lab+".mp3"
...
#				also plays audio files with actions when its necessary
	...
	global online_data_hor,online_data_ver, cont, play_labels, play_times, predicted_times, predicted_labels
	global action_time

	predicted_times = global_variables.get_predicted_times()
	predicted_labels = global_variables.get_predicted_labels()
	cont = global_variables.get_cont()
	...
	num_occ = play_times.count(cont)
	if num_occ > 0:
		pos = play_times.index(cont)
		action_time = cont
		pl_lab = play_labels[pos]
#		play_process = Process(target=play_sound, args=(pl_lab,))
#		play_process.start()
		song_name = "./audio/"+pl_lab+".mp3"
		song = AudioSegment.from_mp3(song_name)
		play(song)
		beep = AudioSegment.from_mp3("./audio/beep.mp3")
		play(beep)
		file = open(global_variables.get_online_aux_file(), 'a')
		to_write = "\n"+str(["TO DO",pl_lab,str(time.localtime().tm_hour)+":"+str(time.localtime().tm_min)+":"+str(time.localtime().tm_sec)])+"\n"
		file.write(to_write)
		file.close()
...
# Every half second we evaluate the data
	#else:
	elif cont == action_time+samples_per_trial: # We are evaluating only 3 seconds after each sound is played
...
# Feature Extraction
	...
	if np.max(probs) > 0.5:
		...
		print("\n\n")
		global_variables.append_predicted_times(global_variables.get_cont())
		global_variables.append_predicted_labels(movement)

		file = open(global_variables.get_online_aux_file(), 'a')
		to_write = str(["PREDICTED",movement,str(time.localtime().tm_hour)+":"+str(time.localtime().tm_min)+":"+str(time.localtime().tm_sec)])+"\n"
		file.write(to_write)
		file.close()
...
def pseudo_onl():
	...
	lab = global_variables.get_lab()
	play_labels = 5*lab
	random.shuffle(play_labels)
	play_labels.append("beep")

	sample_rate = global_variables.get_sample_rate()

# De donde salen estos numeros?
	play_times = list(range(5*sample_rate,len(play_labels)*5*sample_rate+1,5*sample_rate))

	model_name = global_variables.get_model()
	model_name = model_name.replace('.sav','_simulated_online_aux_data')
	pos = model_name.find("/models/")
	file_name = model_name.replace(model_name[0:pos+len("/models/")],"")
	online_aux_files = glob.glob("./data/"+file_name+"*")
	number_online_aux_files = len(online_aux_files)
	online_aux_file_name = file_name+"_"+str(number_online_aux_files+1)
	new_file_name = "./data/"+online_aux_file_name+".txt"
	global_variables.set_online_aux_file(new_file_name)
	file = open(new_file_name, 'w')
	file.close()

	online_data_file_name = file_name.replace('_simulated_online_aux_data', '_online_data')
	online_files = glob.glob("./data/"+online_data_file_name+"*")
	number_online_files = len(online_files)
	online_data_file_name = online_data_file_name+"_"+str(number_online_files+1)
